...
public class Player extends Entity {
	...
	int haskey = 0; // so luong vat pham
	...
	public void pickUpObject(int i) {
	    if (i != 999) { // Kiểm tra nếu có va chạm với một đối tượng
	        String objectName = gp.obj[i].name;
	        switch (objectName) {
	            case "Key":
	                haskey++;
	                gp.obj[i] = null; // Xóa chìa khóa sau khi nhặt
	                System.out.println("Key: " + haskey);
	                break;
	            case "Door":
	                if (haskey > 0) {
	                    gp.obj[i] = null; // Mở cửa nếu người chơi có chìa khóa
	                    haskey--;
